...
add_bold_text(p, '대구 한의대학교 한의학과 졸업')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, '대전대학교 한의학과 대학원 졸업')

p = doc.add_paragraph()
add_normal_text(p, '   • 면허: ')
add_bold_text(p, '한의사 면허 취득 (1994년, 30년+)')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, '美國 한의사 면허 취득 (NCCAOM)')

p = doc.add_paragraph()
add_normal_text(p, '   • 현직: ')
add_bold_text(p, '(現) 소잠한의원 대표원장')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, '대한 통합 방제 한의학회 부회장')
...
p = doc.add_paragraph()
add_normal_text(p, '   • 상태 의학 연구소 고령자 채록사업단 단장')

p = doc.add_paragraph()
add_normal_text(p, '   • (주)동의 한약 분석센터 관능검사 위원')

p = doc.add_paragraph()
add_normal_text(p, '   • 日本 스파이럴 학회 1급 강사 / 韓國 Spiral Taping 학회 교수')
...
p = doc.add_paragraph()
add_normal_text(p, '   • 학력: ')
add_bold_text(p, '호복 중의대 중의학과 졸업')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, 'American Liberty University 명예 한의학 박사')

p = doc.add_paragraph()
add_normal_text(p, '   • 현직: ')
add_bold_text(p, '(現) 상태 의학 연구소 소장')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, '(現) 할아버지 한의학 원장')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, '한방학술 태극학회 학술고문')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, '대한 통합 방제 한의학회 학술고문')

p = doc.add_paragraph()
add_normal_text(p, '   • ')
add_bold_text(p, 'AIMI(미국 통합의학 연구소) 학술고문')

p = doc.add_paragraph()
add_normal_text(p, '   • 영국 초청방문 ME질환 조사, 연구, 치료')

p = doc.add_paragraph()
add_normal_text(p, '   • 북경 중의 연구원 침구배훈중심 학술교류')

p = doc.add_paragraph()
add_normal_text(p, '   • (前) 미국 LA 남가주한의대 초빙교수')
...
# Save
output_path = r'G:\내 드라이브\developer\hanmed-cdss\scripts\temp_docx\2026_초기창업패키지_사업계획서_온고지신AI_FINAL_v2.docx'
