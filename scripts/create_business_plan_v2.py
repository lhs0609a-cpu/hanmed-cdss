...
team_data = [
    ['1', 'CTO', '기술개발 총괄\nAI/ML, 풀스택 개발', '컴퓨터공학 학사, AI/ML 개발 경력 5년+\n플라톤마케팅 대표', '완료'],
...
overview_table.cell(6, 1).text = '''• CEO 양보름: 사업총괄, 전략, 영업 (머프키치 대표)
• CTO 이현석: 기술개발, AI/ML, 풀스택 개발 (플라톤마케팅 대표)
• 의료자문 이○○ 원장: 한의학 자문, 40년 임상 경험, 데이터 검증'''

doc.add_paragraph()

# 이미지 섹션 - 랜딩 & 로그인
doc.add_paragraph('< 서비스 메인 화면 >')
try:
    doc.add_picture(os.path.join(screenshot_dir, '01_landing.png'), width=Inches(6))
...
# 대시보드 스크린샷
doc.add_paragraph()
doc.add_paragraph('< 메인 대시보드 화면 >')
try:
    doc.add_picture(os.path.join(screenshot_dir, '03_dashboard.png'), width=Inches(5.5))
...
# 핵심 기능 스크린샷 페이지
doc.add_heading('< 주요 서비스 화면 >', level=2)

# AI 상담 화면
doc.add_paragraph()
doc.add_paragraph('1. AI 상담 (변증/처방 추천)')
try:
    doc.add_picture(os.path.join(screenshot_dir, '05_consultation.png'), width=Inches(5.5))
    p = doc.add_paragraph('< AI 기반 변증 분석 및 처방 추천 화면 >')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
except:
...
# 통합 검색 화면
doc.add_paragraph('2. 통합 검색 (치험례 검색)')
try:
    doc.add_picture(os.path.join(screenshot_dir, '04_unified_search.png'), width=Inches(5.5))
    p = doc.add_paragraph('< 자연어 기반 치험례 통합 검색 화면 >')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
except:
...
# 추가 스크린샷
doc.add_paragraph('3. 한약재 데이터베이스')
try:
    doc.add_picture(os.path.join(screenshot_dir, '06_herbs.png'), width=Inches(5.5))
...
# 약물 상호작용
doc.add_paragraph('4. 약물 상호작용 검사')
try:
    doc.add_picture(os.path.join(screenshot_dir, '09_interactions.png'), width=Inches(5.5))
    p = doc.add_paragraph('< 양약-한약 상호작용 자동 검사 화면 >')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
except:
...
# 환자 관리
doc.add_paragraph('5. 환자 관리')
try:
    doc.add_picture(os.path.join(screenshot_dir, '07_patients.png'), width=Inches(5.5))
    p = doc.add_paragraph('< 환자 정보 및 진료 이력 관리 화면 >')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
except:
...
# 체질 분석
doc.add_paragraph('6. 사상체질 분석')
try:
    doc.add_picture(os.path.join(screenshot_dir, '08_constitution.png'), width=Inches(5.5))
    p = doc.add_paragraph('< AI 기반 사상체질 분석 화면 >')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
except:
...
# 치험례 관리
doc.add_paragraph('7. 치험례 데이터베이스')
try:
    doc.add_picture(os.path.join(screenshot_dir, '10_cases.png'), width=Inches(5.5))
    p = doc.add_paragraph('< 6,000건+ 치험례 데이터베이스 관리 화면 >')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
except:
...
doc.add_paragraph('''   - CEO 양보름 (머프키치 대표)
     · 역할: 사업총괄, 투자유치, 파트너십 구축, 영업 전략
     · 경력: 머프키치 창업 및 운영 (2022.01~현재)
     · 역량: 사업 개발, 고객 관계 관리, 전략 기획''')

p12 = doc.add_paragraph()
p12.add_run('◦ 핵심 팀원 역량').bold = True

doc.add_paragraph('''   - CTO 이현석 (플라톤마케팅 대표)
     · 학력: 컴퓨터공학 학사
     · 경력: AI/ML 개발 5년+, 플라톤마케팅 대표
...
team_detail_data = [
    ['1', 'CEO', '사업총괄, 영업', '머프키치 대표, 사업 개발 경험', '완료'],
    ['2', 'CTO', '기술개발 총괄', '컴퓨터공학 학사, AI/ML 개발 5년+\n플라톤마케팅 대표', '완료'],
    ['3', '자문위원', '한의학 자문', '한의학 박사, 한의원 원장 40년', '완료'],
    ['4', '개발자', 'Frontend 개발', 'React/TypeScript 경력', "예정('26.3)"]
]
...
# Save
output_path = r'G:\내 드라이브\developer\hanmed-cdss\scripts\temp_docx\2026_초기창업패키지_사업계획서_온고지신AI_v2.docx'
