                ...
                {isExpanded && (
                  <div className="mt-2 space-y-2">
                    {terms.map(([term, info]) => (
                      <div
                        key={term}
                        className="p-3 bg-white border border-gray-100 rounded-xl hover:border-blue-200 hover:bg-blue-50/30 transition-colors"
                      >
    ...
    return (
      <>
        <button
          onClick={() => setIsOpen(true)}
          className={cn(
            'inline-flex items-center gap-1 text-blue-600 hover:text-blue-700 text-sm',
            className
          )}
        >
  ...
  return (
    <>
      <button
        onClick={() => setIsOpen(true)}
        className={cn(
          'inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-xl hover:bg-blue-100 transition-colors text-sm font-medium',
          className
        )}
      >
        <BookOpen className="h-4 w-4" />
      ...
      {processedContent.map((part, index) =>
        part.type === 'term' ? (
          <TermTooltip
            ...
            showIcon={showIcon}
            className={cn(
              'underline decoration-blue-300 decoration-dotted underline-offset-2 cursor-help',
              highlightClassName
            )}
          >
            {part.content}
