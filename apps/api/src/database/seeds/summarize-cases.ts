...
각 필드:
- summaryOneLine: 한 문장. 원문에 적힌 것까지만 쓴다.
  "OO세 O성이 ~로 내원, ~처방 후 ~" 는 결과까지 적혀 있을 때의 형태일 뿐이고,
  틀에 맞추려고 없는 내용을 채우지 않는다. 원문에 나이가 없으면 나이를 빼고,
  결과가 없으면 결과절을 빼고, 내원 여부가 안 적혀 있으면 "내원"을 쓰지 않는다.
  (나쁜 예 — 원문이 "156. 삼기탕 1-2. 탈항 여 51세 주부" 뿐인데
   "51세 여성이 탈항으로 내원, 삼기탕 처방 후 증상 개선" 이라고 쓰는 것.
   원문에 내원도 처방 후 개선도 없다. "삼기탕 탈항 항목 — 여 51세 주부" 가 맞다.)
...
- distinctive: 이 치험례의 특징. 흔한 사례와 무엇이 다른지, 임상의가 자기 환자에
  적용할 때 눈여겨볼 점. 2~3문장. 원문에 진료 서술이 없으면 빈 문자열로 둔다 —
  근거 없이 "~에게 활용 가능성을 고려할 수 있다" 같은 말을 만들지 않는다.
